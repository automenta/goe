...
"""
MAX_VOCAB_SIZE = 2000
MIN_FREQ = 2  # Changed from 1
MAX_LM_SEQ_LEN = 10
TRAIN_SPLIT_RATIO = 0.8

# Model HPs (Defaults) - UPDATED FOR BETTER CAPACITY
EMBED_DIM_DEFAULT = 64  # Was 32
NUM_EXPERTS_DEFAULT = 3
EXPERT_NHEAD_DEFAULT = 4  # Was 2 (ensure embed_dim % nhead == 0)
EXPERT_DIM_FEEDFORWARD_DEFAULT = 256  # Was 64 (embed_dim * 4 is common)
ROUTER_HIDDEN_DIM_DEFAULT = 64  # Was 32
MAX_PATH_LEN_DEFAULT = 4  # Was 3
MAX_VISITS_PER_EXPERT_IN_PATH_DEFAULT = 1

# Training HPs
LEARNING_RATE = 1e-3
BATCH_SIZE = 8
ROUTER_ENTROPY_COEF = 0.015  # Was 0.01
...
class Vocabulary:
    ...
    def add_word(self, word):
        # Allow adding even if finalized, for building from multiple sources,
        # but build_vocab will reset if it's a true fresh build.
        self.word_counts[word] += 1

    def build_vocab(self):
        # Only truly rebuild if not finalized or if it's an empty vocab shell
        if self._finalized and len(self.word_to_idx) > 4: return

        current_special_tokens = self.word_to_idx.copy()  # Preserve if any were set

        sorted_words = self.word_counts.most_common()

        # Reset for a clean build, keeping essential special tokens
        self.word_to_idx = {"<pad>": 0, "<unk>": 1, "<bos>": 2, "<eos>": 3}
        # Add back any other special tokens that might have been there
        for token, idx in current_special_tokens.items():
            if token not in self.word_to_idx:
                self.word_to_idx[token] = len(self.word_to_idx)  # Add with new index if needed

        self.idx_to_word = {v: k for k, v in self.word_to_idx.items()}

        for word, count in sorted_words:
            if self.max_size and len(self.word_to_idx) >= self.max_size: break
            if count < self.min_freq and word not in self.word_to_idx: continue  # Respect min_freq
            if word not in self.word_to_idx:  # Add only new words
                idx = len(self.word_to_idx)
                self.word_to_idx[word] = idx
                self.idx_to_word[idx] = word
        self._finalized = True
        print(f"Vocabulary built with {len(self.word_to_idx)} words. Min Freq: {self.min_freq}")
...
class LanguageModelDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Denumericalize for animation/logging
        # Ensure vocab is available and has idx_to_word mapping
        try:
        ...
        except Exception as e:  # Fallback if vocab is weird during early init
            # print(f"Warning: Vocab issue during getitem: {e}")
...
class RoutingController(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_experts):
        ...
        self.relu = nn.ReLU()  # Explicitly using F.relu in forward
    ...
    def forward(self, x_summary, current_visit_counts_for_batch_item, max_visits_per_expert):
        ...
        # Iterate over batch size or 1 if not batched
        for i in range(logits.size(0) if is_batched else 1):
            # Select correct visit counts and logits slice
            visits = current_visit_counts_for_batch_item[i] if is_batched else current_visit_counts_for_batch_item
            current_logits_slice = logits[i] if is_batched else logits

            for expert_idx in range(self.num_experts):
                if visits[expert_idx] >= max_visits_per_expert:
                    current_logits_slice[expert_idx] = -float('inf')  # Modify slice or original tensor
        return logits
...
class GoEModel(nn.Module):
    def __init__(self, vocab_size, embed_dim, num_experts, expert_nhead, expert_dim_feedforward,
        ...
        self.pos_encoder = PositionalEncoding(embed_dim, max_len=max_lm_seq_len + 5)  # Max_len for PE
        ...
        self.current_hps = {}  # Store HPs, especially for animation_delay
    ...
    def forward(self, input_ids_seq, current_sample_text_for_anim=None):
        ...
        for step in range(self.max_path_len):
            ...
            step_entropy = -torch.sum(router_probs * torch.log(router_probs.clamp(min=1e-9)), dim=1).mean()
        ...
        if self.is_animating_sample:
            ...
            predicted_token_str = "UNK_ANIM"  # Fallback
...
class DiagramWidget(QWidget):
    ...
    def _setup_node_positions(self):
        ...
        expert_y = H * 0.60
        # Ensure num_experts_to_draw is at least 1 for division safety, though UI constrains to >=2
        denominator = max(1, self.num_experts_to_draw - 1)
        for i in range(self.num_experts_to_draw):
            x_pos = W * (0.15 + (0.7 / denominator) * i if self.num_experts_to_draw > 1 else 0.5)
        ...
        self.base_connections = [("INPUT", "EMBED"), ("EMBED", "ROUTER")] + \
                                [("ROUTER", f"EXPERT_{i}") for i in range(self.num_experts_to_draw)] + \
    ...
    def highlight_connection(self, from_name, to_name, active=True):
        ...
        for i, (s, d, c, _) in enumerate(self.connections_to_draw):
            if s == from_name and d == to_name:
                ...
                updated_conn = True
                break
        if not updated_conn:  # Should ideally not happen if base_connections is complete
    ...
    def set_router_probs_text(self, p_list):
        if not p_list: self.router_probs_text = ""; return
        labels = ",".join([f"E{i}" for i in range(self.num_experts_to_draw)]) + ",Term"
        self.router_probs_text = f"Router({labels}): [{','.join([f'{x:.2f}' for x in p_list])}]";
        self.update()

    def set_current_path_text(self, p_list):
        self.current_path_text = "Path: " + "->".join(map(str, p_list)); self.update()

    def paintEvent(self, event):
        painter = QPainter(self);
        painter.setRenderHint(QPainter.Antialiasing)
        for s_name, d_name, col, is_active_conn in self.connections_to_draw:
            ...
            painter.setPen(QPen(col, 2.5 if is_active_conn else 1.5));
            painter.drawLine(p1, p2)
            angle = math.atan2(p2.y() - p1.y(), p2.x() - p1.x());
            arr_sz = 7.0  # Arrow size
            arr_p1 = QPointF(p2.x() - arr_sz * math.cos(angle - math.pi / 7),
                             p2.y() - arr_sz * math.sin(angle - math.pi / 7))  # Arrow points
...
class ContinuousLearningWorker(QThread):
    def __init__(self, model_provider_fn, data_provider_fn, hps, signals_obj, parent=None):
        ...
        self.hps = hps  # This HPS should be a copy from main thread at worker creation
    ...
    def run(self):
        ...
        if not model or not train_loader or not val_loader:  # Check val_loader too
            self.signals.log_message.emit("ERROR: Worker: Model or Dataloader(s) not available.")
            self._is_running = False
            self.signals.learning_stopped.emit()
            return

        model.current_hps = self.hps  # Pass HPs to model for animation delay access
        ...
        while self._is_running:
            if self._pause_for_animation and self._animate_this_sample_info:
                ...
                QThread.msleep(50)  # Brief pause after animation
            ...
            # Entropy term: we want to MAXIMIZE entropy H = -sum(p log p).
            # So, in loss, it's Loss_task - coeff * H.
            # router_entropy is already H (positive).
            loss = task_loss - self.hps['router_entropy_coef'] * router_entropy
            loss.backward()
            # Gradient Clipping
            torch.nn.utils.clip_grad_norm_(model.parameters(),
                                           max_norm=self.hps.get('gradient_clip_norm', GRADIENT_CLIP_NORM))
            optimizer.step()
            ...
            if self.global_step % self.hps['train_steps_per_eval'] == 0:
                ...
                with torch.no_grad():
                    for val_input_ids, val_target_ids, _ in val_loader:
                        ...
                        for p_val in val_paths: path_lengths_this_eval.append(
                ...
                perplexity = math.exp(avg_val_loss) if avg_val_loss != float('inf') and avg_val_loss < 700 else float(
                    'inf')  # Cap perplexity for display
                ...
                stats_payload = {
                    ...
                    'train_loss_hist': list(self.train_loss_history), 'val_loss_hist': list(self.val_loss_history),
                    ...
                    'expert_usage': dict(self.expert_usage_counts), 'current_val_loss': avg_val_loss,
                    'current_perplexity': perplexity
                }
                self.signals.stats_update.emit(stats_payload)

            QThread.msleep(1)  # Minimal sleep
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self._init_ui()  # This will set default HPs to UI elements
        self._connect_signals_basic()
        self.init_system_data_and_model()  # Initial setup with UI defaults
    ...
    def _get_default_hps(self):
        return {'embed_dim': EMBED_DIM_DEFAULT, 'num_experts': NUM_EXPERTS_DEFAULT,
                ...
                'router_hidden_dim': ROUTER_HIDDEN_DIM_DEFAULT, 'max_path_len': MAX_PATH_LEN_DEFAULT,
                'max_visits_per_expert': MAX_VISITS_PER_EXPERT_IN_PATH_DEFAULT, 'max_lm_seq_len': MAX_LM_SEQ_LEN,
                'learning_rate': LEARNING_RATE, 'batch_size': BATCH_SIZE, 'router_entropy_coef': ROUTER_ENTROPY_COEF,
                'weight_decay': WEIGHT_DECAY, 'animation_delay': ANIMATION_DELAY_DEFAULT,
                'train_steps_per_eval': TRAIN_STEPS_PER_EVAL, 'gradient_clip_norm': GRADIENT_CLIP_NORM}
    ...
    def _get_current_hps_from_ui(self):
        return {'embed_dim': self.embed_dim_spin.value(), 'num_experts': self.num_experts_spin.value(),
                ...
                'gradient_clip_norm': GRADIENT_CLIP_NORM}  # Added gradient_clip_norm
    ...
    def _init_ui(self):
        ...
        # Ensure these defaults match the constants
        ...
        self.expert_nhead_spin.setValue(EXPERT_NHEAD_DEFAULT)  # Ensure embed_dim % nhead == 0
        self.expert_ff_spin = QSpinBox();
        self.expert_ff_spin.setRange(32, 512);
        ...
        if PYQTGRAPH_AVAILABLE:
            pg.setConfigOptions(antialias=True, background=(240, 240, 240), foreground='k')  # Lighter background
            self.loss_plot_widget = pg.PlotWidget(title="Loss Curves")
            self.loss_plot_widget.addLegend(offset=(-10, 10), brush=(200, 200, 200, 150), labelTextColor='k')
            ...
            self.loss_plot_widget.setLabel('bottom', "Evaluation Steps x EvalFreq")
            ...
            self.perplexity_plot_widget.setLabel('bottom', "Evaluation Steps x EvalFreq")
            plots_layout.addWidget(self.perplexity_plot_widget)

            self.aux_metrics_plot_widget = pg.PlotWidget(title="Auxiliary Metrics")
            self.aux_metrics_plot_widget.addLegend(offset=(-10, 10), brush=(200, 200, 200, 150), labelTextColor='k')
            ...
            self.aux_metrics_plot_widget.setLabel('bottom', "Evaluation Steps x EvalFreq")
    ...
    def init_system_data_and_model(self):
        ...
        if self.learning_worker and self.learning_worker.isRunning():
            ...
            self.stop_continuous_learning()  # This sets worker._is_running = False
            if not self.learning_worker.wait(3000):  # Wait up to 3s
                self.log_message("Warning: Learning worker did not terminate gracefully. Forcing quit.")
                self.learning_worker.terminate()  # Force if necessary
                self.learning_worker.wait()  # Wait again after terminate
            self.learning_worker = None  # Clear old worker
        ...
        self.vocab = Vocabulary(max_size=MAX_VOCAB_SIZE, min_freq=self.hps.get('min_freq', MIN_FREQ))
        ...
        self.val_loader = torch.utils.data.DataLoader(self.val_dataset, batch_size=self.hps['batch_size'],
                                                      shuffle=False)
        self.log_message(
            f"Data: Train {len(self.train_dataset)} seqs, Val {len(self.val_dataset)} seqs. Vocab: {len(self.vocab)}.")
    ...
    def start_continuous_learning(self):
        ...
        # Pass a copy of HPs to the worker so it's fixed at worker creation time
        worker_hps = self.hps.copy()
    ...
    def trigger_one_sample_animation(self):
        ...
        self.log_message(f"Requesting animation for val sample: {text_snippet[:50]}...")
        # Pass current HPs for animation delay to the model via the worker
    ...
    def handle_stats_update(self, stats_payload):
        ...
        if not found_candidates: self.pathway_stats_text_edit.append("No pathways meet >5% usage.")
    ...
    def update_plots_from_cache(self):
        ...
        x_axis_data = list(range(len(stats.get('val_loss_hist', []))))  # Use length of val_loss as x-axis

        self.train_loss_curve.setData(x=list(range(len(stats.get('train_loss_hist', [])))),
                                      y=stats.get('train_loss_hist', []))
        self.val_loss_curve.setData(x=x_axis_data, y=stats.get('val_loss_hist', []))
        self.perplexity_curve.setData(x=x_axis_data, y=stats.get('val_perp_hist', []))
        self.path_len_curve.setData(x=x_axis_data, y=stats.get('path_len_hist', []))
        self.router_entropy_curve.setData(x=x_axis_data, y=stats.get('router_entropy_hist', []))

        expert_usage = stats.get('expert_usage', {});
        num_defined_experts = self.hps.get('num_experts', NUM_EXPERTS_DEFAULT)
        x_ticks_labels = [];
        heights = []
        for i in range(num_defined_experts): x_ticks_labels.append((i, f"E{i}")); heights.append(expert_usage.get(i, 0))
    ...
    def on_to_router_anim(self, r, p):
        ...
        if p and p[-1] != 'T' and p[-1] != 'T_max' and isinstance(p[-1],
                                                                  int):  # Check if last element is an expert index
            last_expert_idx = p[-1]
