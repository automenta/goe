...
"""
MAX_VOCAB_SIZE = 10000  # Increased for richer vocabulary
MIN_FREQ = 1
MAX_LM_SEQ_LEN = 30  # Increased for longer context
TRAIN_SPLIT_RATIO = 0.8

# Model Hyperparameters
EMBED_DIM_DEFAULT = 256  # Increased for capacity
NUM_EXPERTS_DEFAULT = 6  # More experts for diversity
EXPERT_NHEAD_DEFAULT = 4
EXPERT_DIM_FEEDFORWARD_DEFAULT = 1024  # Increased for depth
ROUTER_HIDDEN_DIM_DEFAULT = 256
MAX_PATH_LEN_DEFAULT = 8
MAX_VISITS_PER_EXPERT_DEFAULT = 2
GUMBEL_TAU_DEFAULT = 1.5  # Higher for initial exploration
DIVERSITY_LOSS_COEF = 0.005  # Reduced to balance
CONTRASTIVE_LOSS_COEF = 0.01

# Training Hyperparameters
LEARNING_RATE = 3e-4  # Further lowered
BATCH_SIZE = 32  # Increased
ROUTER_ENTROPY_COEF = 0.01  # Adjusted
GRADIENT_CLIP_NORM = 0.3  # Tighter
WEIGHT_DECAY = 5e-6
TRAIN_STEPS_PER_EVAL = 20
SMOOTHING_WINDOW_LOSS = 10
WARMUP_STEPS = 200
TOTAL_STEPS = 15000  # Extended training

# UI Constants
ANIMATION_DELAY_DEFAULT = 0.2
PLOT_UPDATE_INTERVAL_MS = 300
MAX_PLOT_POINTS = 300
DIAGRAM_NODE_WIDTH = 110
DIAGRAM_NODE_HEIGHT = 40
DIAGRAM_ARROW_SIZE = 9.0
...
def augment_text(text: str) -> str:
    ...
    augmented = []
    for token in tokens:
        if random.random() < 0.15:  # 15% chance to synonym swap
            synonyms = {
                "game": "title", "tactical": "strategic", "unit": "team", "mission": "operation",
                "developed": "created", "released": "launched", "story": "narrative"
            }
    ...
    return " ".join(augmented) + " " + text.lower()
...
class Expert(nn.Module):
    def __init__(self, embed_dim: int, nhead: int, dim_feedforward: int, dropout: float = 0.2):
        super().__init__()
        self.layers = nn.ModuleList([
            nn.TransformerEncoderLayer(
                d_model=embed_dim, nhead=nhead, dim_feedforward=dim_feedforward,
                dropout=dropout, batch_first=True
            ) for _ in range(3)  # Three layers for depth
        ])
...
class RoutingController(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int, num_experts: int):
        ...
        self.fc1 = nn.Linear(input_dim, hidden_dim // 2)  # Bottleneck
        self.fc2 = nn.Linear(hidden_dim // 2, hidden_dim)
        self.fc3 = nn.Linear(hidden_dim, num_experts + 1)
...
class PathOptimizer(nn.Module):
    def __init__(self, embed_dim: int, num_experts: int, max_path_len: int):
        super().__init__()
        self.rnn = nn.LSTM(embed_dim, embed_dim // 2, batch_first=True)
        self.fc = nn.Linear(embed_dim // 2, num_experts + 1)  # +1 for termination

    def forward(self, summary_repr: torch.Tensor, path_indices: List[List[Any]]) -> torch.Tensor:
        batch_size = summary_repr.size(0)
        max_len = max(len(p) for p in path_indices)
        path_tensor = torch.zeros(batch_size, max_len, dtype=torch.long, device=summary_repr.device)
        for i, path in enumerate(path_indices):
            for j, step in enumerate(path):
                path_tensor[i, j] = step if isinstance(step, int) else (self.fc.out_features - 1)
        h0 = torch.zeros(1, batch_size, summary_repr.size(1) // 2, device=summary_repr.device)
        c0 = torch.zeros(1, batch_size, summary_repr.size(1) // 2, device=summary_repr.device)
        out, _ = self.rnn(summary_repr.unsqueeze(1).expand(-1, max_len, -1), (h0, c0))
        logits = self.fc(out)
        loss = F.cross_entropy(logits.view(-1, logits.size(-1)), path_tensor.view(-1), ignore_index=self.fc.out_features - 1)
        return loss
...
class GoEModel(nn.Module):
    def __init__(
        ...
        self.path_optimizer = PathOptimizer(embed_dim, num_experts, max_path_len)
        self.animation_signals_obj = None
        self.is_animating_sample = False
        self.current_hps = {}
        self.expert_outputs = []  # For contrastive loss
    ...
    def compute_contrastive_loss(self) -> torch.Tensor:
        if len(self.expert_outputs) < 2:
            return torch.tensor(0.0, device=self.expert_outputs[0].device if self.expert_outputs else torch.device('cpu'))
        loss = 0.0
        for i in range(len(self.expert_outputs)):
            for j in range(i + 1, len(self.expert_outputs)):
                cos_sim = F.cosine_similarity(self.expert_outputs[i].mean(dim=1), self.expert_outputs[j].mean(dim=1), dim=-1)
                loss += cos_sim.mean()  # Minimize similarity
        return loss / (len(self.expert_outputs) * (len(self.expert_outputs) - 1) / 2)
    ...
    def forward(self, input_ids_seq: torch.Tensor, current_sample_text: Optional[str] = None) -> Tuple[torch.Tensor, torch.Tensor, List[List[Any]], torch.Tensor]:
        ...
        total_aux_loss = torch.tensor(0.0, device=input_ids_seq.device)
        ...
        self.expert_outputs = []
        epsilon = 1e-10
        tau = self.gumbel_tau * (1 - self.current_hps.get('global_step', 0) / TOTAL_STEPS) + 0.5  # Dynamic tau
        for step in range(self.max_path_len):
            ...
            total_aux_loss += self.compute_diversity_loss(router_probs)
            one_hot_decision = F.gumbel_softmax(router_logits, tau=tau, hard=True) if self.training else F.one_hot(
        ...
        if self.training:
            path_loss = self.path_optimizer(final_summary_repr, paths_taken_indices)
            contrastive_loss = self.compute_contrastive_loss()
            total_aux_loss += path_loss + CONTRASTIVE_LOSS_COEF * contrastive_loss
        else:
            total_aux_loss += torch.tensor(0.0, device=input_ids_seq.device)
        if self.is_animating_sample:
        ...
        return logits, total_router_entropy, paths_taken_indices, total_aux_loss
...
class DiagramWidget(QWidget):
    ...
    def set_path_diversity_text(self, path_freq: Dict[Tuple, int]):
        total = sum(path_freq.values()) + 1e-10
        probs = torch.tensor([count / total for count in path_freq.values()])
        entropy = -torch.sum(probs * torch.log(probs + 1e-10)).item()
        self.path_diversity_text = f"Path Diversity: {entropy:.2f}"
        self.update()
    ...
    def paintEvent(self, event):
        ...
        painter.drawText(QPointF(5, 95), self.path_diversity_text)
...
class ContinuousLearningWorker(QThread):
    def __init__(
        ...
        self.path_diversity_history = collections.deque(maxlen=MAX_PLOT_POINTS)
        self.global_step = 0
    ...
    def run(self):
        ...
        scheduler = CosineAnnealingLR(optimizer, T_max=TOTAL_STEPS - WARMUP_STEPS, eta_min=5e-7)
        train_iter = iter(train_loader)
        recent_losses = collections.deque(maxlen=SMOOTHING_WINDOW_LOSS)
        entropy_coef = self.hps['router_entropy_coef']
        curriculum_threshold = 0.5 * TOTAL_STEPS  # Curriculum phase
        for step in range(TOTAL_STEPS):
            ...
            model.train()
            optimizer.zero_grad()
            current_entropy_coef = entropy_coef * (1 - step / TOTAL_STEPS)
            model.current_hps['global_step'] = step
            ...
            if self.global_step % self.hps['train_steps_per_eval'] == 0:
                ...
                total_paths = sum(self.pathway_frequencies.values()) + 1e-10
                path_probs = torch.tensor([count / total_paths for count in self.pathway_frequencies.values()])
                path_diversity = -torch.sum(path_probs * torch.log(path_probs + 1e-10)).item()
                ...
                stats_payload = {
                    ...
                    'path_diversity_hist': list(self.path_diversity_history),
...
class MainWindow(QMainWindow):
    ...
    def _init_ui(self):
        ...
        self.entropy_coef_spin.setRange(0.0, 0.02)
        ...
        self.train_eval_steps_spin.setRange(10, 50)
        ...
        if PYQTGRAPH_AVAILABLE:
            ...
            self.path_diversity_curve = self.aux_metrics_plot_widget.plot(pen=pg.mkPen('y', width=2), name="Path Diversity")
    ...
    def init_system_data_and_model(self):
        ...
        for _ in range(3):  # Triple augmentation
    ...
    def reset_plots(self):
        ...
        self.path_diversity_curve.clear()
    ...
    def handle_stats_update(self, stats_payload: Dict[str, Any]):
        ...
        self.diagram_widget.set_path_diversity_text(stats_payload.get('pathway_freq', {}))
    ...
    def update_plots_from_cache(self):
        ...
        self.path_diversity_curve.setData(x=x_axis, y=stats.get('path_diversity_hist', []))
