...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model: int, dropout: float = 0.1, max_len: int = 5000):
        ...
        self.register_buffer('pe', pe.unsqueeze(0))  # pe will be moved with model.to(device)
...
class Expert(nn.Module):
    def __init__(self, embed_dim: int, nhead: int, dim_ff: int, num_layers: int = 4, dropout: float = 0.1):
        ...
        self.tag = nn.Parameter(torch.randn(1, 1, embed_dim) * 0.01)  # Will be moved with model.to(device)
...
class RoutingController(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int, num_experts: int):
        ...
        self.q_values = nn.Parameter(torch.zeros(num_experts + 1))  # Will be moved with model.to(device)
    ...
    def forward(self, x_summary: torch.Tensor, visit_counts: torch.Tensor, max_visits: int) -> torch.Tensor:
        ...
        extended_mask = torch.cat([mask, torch.zeros_like(mask[:, :1])],
                                  dim=-1)  # Termination action not masked by visits
        # Ensure q_values is on the same device as logits for addition
...
class RunningStat:
    ...
    def update(self, x_tensor: torch.Tensor):
        ...
        for x in x_tensor.flatten().tolist():  # Operates on CPU numbers
            self.count += 1
            delta = x - self.mean
    ...
    def normalize(self, x: torch.Tensor) -> torch.Tensor:  # x is a tensor, op should be on its device
        # self.mean and self.std are floats, conversion to tensor handled by PyTorch
...
class GoEModel(nn.Module):
    def __init__(self, vocab_size: int, embed_dim: int, num_experts: int, expert_nhead: int,
                 ...
                 gumbel_tau_init: float):  # Renamed max_lm_seq_len to avoid conflict
        super().__init__()
        ...
        # max_len in PositionalEncoding is for its internal buffer size, not tied to input seq len directly
        ...
        self.path_optimizer = nn.Linear(embed_dim, self.max_path_len + 1)  # Predicts path length up to max_path_len
    ...
    @property
    def model_device(self) -> torch.device:  # Helper to get current model device
    ...
    def _compute_diversity_loss(self, router_probs: torch.Tensor, expert_usage: Dict[int, int]) -> torch.Tensor:
        ...
        usage_prop = torch.tensor([expert_usage.get(i, 0) / total_usage for i in range(self.num_experts)],
                                  device=router_probs.device)  # Use router_probs.device for new tensor
    ...
    def _compute_q_loss(self, logits: torch.Tensor, rewards: torch.Tensor, discount: float) -> torch.Tensor:
        q_router = self.router.q_values.to(logits.device)  # Ensure q_values on correct device
    ...
    def _compute_contrastive_loss(self, outputs: List[torch.Tensor]) -> torch.Tensor:
        ...
        # Ensure all outputs are on model_dev before stacking, though they should be if experts are.
        means = torch.stack([o.mean(dim=1).squeeze(0) for o in outputs])
        if means.size(0) < 2: return torch.tensor(0.0, device=model_dev)  # means is on device of o

        normed = F.normalize(means, p=2, dim=1)
        sim = torch.matmul(normed, normed.t())
        target = torch.eye(means.size(0), device=sim.device)  # Target on same device as sim
    ...
    def forward(self, input_ids: torch.Tensor, current_sample_text: Optional[str] = None,
        ...
        current_device = input_ids.device  # Use device of input_ids
        ...
        tau = max(self.gumbel_tau_init * (1 - step / total_steps), 0.1)  # Anneal tau
        discount = 0.9 + 0.09 * (step / total_steps)  # Anneal discount factor
        ...
        for _ in range(self.max_path_len):
            ...
            anim_idx_0 = 0  # For animation, always refers to the 0-th sample of the original batch
            is_anim = self.is_animating_sample and (
                        anim_idx_0 in active_orig_idx.tolist())  # Check if the 0-th sample is still active
            if is_anim:
                anim_curr_idx = (active_orig_idx == anim_idx_0).nonzero(as_tuple=True)[
                    0].item()  # Find its current index in active_x
            ...
            hard = self.training  # Use hard Gumbel-Softmax for training, argmax for eval
            ...
            step_logs.append((rtr_logits.clone(), active_orig_idx.clone()))  # For Q-loss later
        ...
        if active_orig_idx.numel():  # Samples that reached max_path_len without terminating
        ...
        if self.training and target_ids is not None:
            path_pred = self.path_optimizer(
                final_summary.detach())  # Detach to not affect final_summary gradients for LM task
            ...
            lm_rewards = -F.cross_entropy(logits.detach(), target_ids,
                                          reduction='none')  # Detach logits for reward calculation
            self.reward_stat.update(lm_rewards.cpu())  # RunningStat works with CPU floats
            rewards_norm = self.reward_stat.normalize(lm_rewards).clamp(-1.0, 1.0)

            for rtr_lgts, act_idx in step_logs:  # rtr_lgts and act_idx are from previous loop iterations
                if act_idx.numel() > 0:
                    # Ensure rewards_norm[act_idx] is on the same device as rtr_lgts
                    total_q += self._compute_q_loss(rtr_lgts, rewards_norm[act_idx].to(rtr_lgts.device), discount)

        if self.is_animating_sample:  # anim_idx_0 is the original 0-th sample
...
class DiagramWidget(QWidget):
    def __init__(self, num_experts: int = NUM_EXPERTS_DEFAULT):
        super().__init__()
        self.setMinimumSize(350, 250)  # Reduced minimum size for better adaptability
    ...
    def _setup_node_positions(self):
        W, H = self.width() or 400, self.height() or 300  # Use smaller defaults if width/height is 0
        ...
        for i in range(self.num_experts):
            ...
            radius_x = W * 0.35
            radius_y = H * 0.15  # Elliptical layout for experts
        ...
        self.base_connections = [("INPUT", "EMBED"), ("EMBED", "ROUTER")] + \
                                ...
                                [("ROUTER", "LM_HEAD")]  # Connection from router to LM_HEAD (for termination)
        self.reset_highlights()
    ...
    def highlight_connection(self, from_n: str, to_n: str, active: bool = True):
        ...
        for i, (s, d, _, _) in enumerate(self.connections_to_draw):
            if s == from_n and d == to_n:
                ...
                updated = True
                break
        if not updated:  # Should not happen if base_connections are comprehensive
            self.connections_to_draw.append((from_n, to_n, QColor("orange") if active else QColor("lightgray"), active))

        if active:  # Deactivate other outgoing connections from the source node if this one is becoming active
            for i, (s, d, col, is_act) in enumerate(self.connections_to_draw):
                if s == from_n and d != to_n and is_act:  # If it's an outgoing from source, not this target, and was active
    ...
    def set_current_path_text(self, p: List[Any]):
        self.current_path_text = "Path: " + "->".join(map(str, p))
        self.update()
...
class AnimationSignals(QWidget):  # QWidget for pyqtSignal
    ...
    def __init__(self, parent: Optional[QWidget] = None):  # Vocab will be set later
        super().__init__(parent)
        self.vocab: Optional[Vocabulary] = None


class ContinuousLearningSignals(QWidget):  # QWidget for pyqtSignal
...
class ContinuousLearningWorker(QThread):
    ...
    def run(self):
        ...
        model, optimizer, criterion = self.model_provider()  # Model is already on DEVICE from provider
        ...
        while self.step < TOTAL_STEPS and self._is_running:
            ...
            if self._pause_anim and self._anim_info:
                ...
                with torch.no_grad(): model(ids_anim.unsqueeze(0).to(DEVICE),
                                            current_sample_text=txt_anim)  # Ensure anim data to device
            ...
            in_ids, tgt_ids = in_ids.to(DEVICE), tgt_ids.to(DEVICE)  # Move data to device

            model.train()
            ent_coef_val = max(self.hps['router_entropy_coef'] * (1 - self.step / max(1, TOTAL_STEPS)), 0.001)
            ...
            if accum_grads_count >= self.grad_accum_steps:
                ...
                # Dynamic LR adjustment based on grad norm (simple version)
                if grad_norm_val > 2.5 * clip_val and current_lr_factor > 0.1:  # Stricter condition
                    current_lr_factor = max(0.1, current_lr_factor * LR_FACTOR)  # Slower reduction
                    self.signals.log_message.emit(
                        f"High grad norm: {grad_norm_val:.2f}, LR factor: {current_lr_factor:.2f}")
                for group in optimizer.param_groups: group['lr'] = self.hps['learning_rate'] * current_lr_factor
            ...
            if logits.numel() > 0 and tgt_ids.numel() > 0:
                self.reward_hist.append(-F.cross_entropy(logits.detach(), tgt_ids, reduction='none').mean().item())
            ...
            if self.step % self.hps['train_steps_per_eval'] == 0:
                ...
                with torch.no_grad():
                    for val_in_ids, val_tgt_ids, _ in val_loader:
                        val_in_ids, val_tgt_ids = val_in_ids.to(DEVICE), val_tgt_ids.to(DEVICE)  # Val data to device
                ...
                val_ppl = math.exp(min(avg_val_loss, 700))  # Cap loss for perplexity
            ...
            QThread.msleep(1)  # Brief pause to allow GUI to process events
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setGeometry(30, 30, 1400, 900)  # Adjusted default size
        self.hps = self._get_default_hps()
        self.vocab: Optional[Vocabulary] = None  # Initialized in init_system_data_and_model
        ...
        self.init_system_data_and_model()  # Initial call to setup
    ...
    def _get_default_hps(self) -> Dict[str, Any]:  # Keep only UI-configurable HPs here
    ...
    def _get_current_hps_from_ui(self) -> Dict[str, Any]:  # Augment with non-UI HPs
        ui_hps = {name: spin.value() for name, spin in self.hps_spins.items()}
        # Add non-UI HPs needed by model/worker
        ui_hps.update({
            'max_lm_seq_len': MAX_LM_SEQ_LEN,  # From constants
            'weight_decay': WEIGHT_DECAY,  # From constants
            'gradient_clip_norm': GRADIENT_CLIP_NORM,  # From constants
            'global_step': self.worker.step if self.worker else 0  # Get current step if worker exists
        })
        return ui_hps
    ...
    def validate_hps(self, hps_to_validate: Dict[str, Any]) -> List[str]:
        ...
        if self.train_ds and hps_to_validate['batch_size'] > len(self.train_ds.sequences):  # Check train_ds exists
            errs.append(f"BatchSz > train seqs ({len(self.train_ds.sequences)})")
        if EFFECTIVE_BATCH_SIZE % hps_to_validate['batch_size'] != 0: errs.append(
    ...
    def _init_ui(self):
        ...
        left_scroll_area.setWidgetResizable(True)  # Important for layout to fill scroll area
        left_panel_widget_container = QWidget()  # This widget will contain the layout
        left_panel_layout = QVBoxLayout(left_panel_widget_container)  # Apply layout to the container
        left_panel_layout.setSpacing(10)

        hps_group = QGroupBox("Hyperparameters")
        hps_form = QFormLayout()
        self.hps_spins = {
            'num_experts': self._create_spinbox(1, 16, NUM_EXPERTS_DEFAULT),  # Increased max experts
            'embed_dim': self._create_spinbox(64, 1024, EMBED_DIM_DEFAULT, step=32),  # step for easier multiples
            ...
            'gumbel_tau': self._create_spinbox(0.1, 5.0, GUMBEL_TAU_INIT, 0.1, 2),  # Added decimals
            ...
            'train_steps_per_eval': self._create_spinbox(5, 200, TRAIN_STEPS_PER_EVAL,
                                                         step=5)}  # Increased eval steps range
        ...
        left_panel_layout.addStretch(1)  # Pushes content up if space allows

        left_scroll_area.setWidget(left_panel_widget_container)  # Set the container widget for scroll area
        top_layout.addWidget(left_scroll_area, stretch=1)  # Add scroll area to main layout
        ...
        if PYQTGRAPH_AVAILABLE:
            ...
            self.path_viz.setCameraPosition(distance=40, elevation=20, azimuth=45)  # Adjusted camera
            plots_layout.addWidget(self.path_viz, stretch=1)  # Allow 3D view to stretch
        else:
        ...
        if PYQTGRAPH_AVAILABLE:
            self.expert_usage_plot = pg.PlotWidget()
            self.expert_bars = None
            stats_layout.addWidget(self.expert_usage_plot, stretch=1)
        else:
    ...
    def update_nhead_options(self):  # Ensure nhead is always a divisor of embed_dim
        emb_dim_val = self.hps_spins['embed_dim'].value()
        nhead_spin = self.hps_spins['expert_nhead']
        max_allowable_nhead = emb_dim_val  # nhead can be up to embed_dim
        ...
        if valid_nheads:
            nhead_spin.setRange(min(valid_nheads), max(valid_nheads))  # Set sensible range
            if current_nhead_val not in valid_nheads:  # If current value invalid, pick closest or first valid
                nhead_spin.setValue(
        ...
        else:  # Fallback if no valid divisors (e.g., embed_dim is prime, nhead_max is 1)
            nhead_spin.setRange(1, 1)
    ...
    def init_system_data_and_model(self):
        ...
        if self.worker and self.worker.isRunning():
            ...
            if not self.worker.wait(3000):
                self.log_message("Warn: Worker did not terminate gracefully. Forcing.")
                self.worker.terminate()
                self.worker.wait()
            self.worker = None

        self.hps = self._get_current_hps_from_ui()  # Get HPs from UI
        if (errors := self.validate_hps(self.hps)):
            self.log_message(f"ERROR: Invalid HPs: {'; '.join(errors)}")
            return

        tokens = tokenize_text(WIKITEXT_SAMPLE)
        for _ in range(3): tokens.extend(tokenize_text(augment_text(WIKITEXT_SAMPLE)))  # Augment data
        split_idx = int(len(tokens) * TRAIN_SPLIT_RATIO)
        train_tok, val_tok = tokens[:split_idx], tokens[split_idx:]

        self.vocab = Vocabulary(MAX_VOCAB_SIZE, MIN_FREQ)  # Create new vocab
        self.train_ds = LanguageModelDataset(train_tok, self.vocab, self.hps['max_lm_seq_len'], "train")
        self.anim_signals.vocab = self.vocab  # Update vocab ref in animation signals
        ...
        if not self.train_ds.sequences or not self.val_ds.sequences:
            self.log_message("ERROR: No sequences generated from data. Check MAX_LM_SEQ_LEN and data size.")
            self.start_btn.setEnabled(False)
            return

        # DataLoader: pin_memory True is good for GPU. num_workers > 0 might cause issues on Windows.
        ...
        self.val_dl = torch.utils.data.DataLoader(self.val_ds, batch_size=self.hps['batch_size'], shuffle=False,
                                                  **dl_args)
        self.log_message(
            f"Data: Train {len(self.train_ds)}, Val {len(self.val_ds)} sequences. Vocab size: {len(self.vocab)}.")
        ...
        self.model.to(DEVICE)  # Move model to selected device
        self.model.animation_signals_obj = self.anim_signals  # Pass animation signal emitter

        self.optim = optim.AdamW(self.model.parameters(), lr=self.hps['learning_rate'],
                                 weight_decay=self.hps['weight_decay'], eps=1e-8)
        self.crit = nn.CrossEntropyLoss(ignore_index=self.vocab.word_to_idx["<pad>"]).to(DEVICE)  # Loss also to device
    ...
    def start_continuous_learning(self):
        ...
        self.worker.start()
        if PYQTGRAPH_AVAILABLE and hasattr(self, 'plot_timer') and not self.plot_timer.isActive():
            self.plot_timer.start(PLOT_UPDATE_INTERVAL_MS)
        self.start_btn.setEnabled(False)
        self.stop_btn.setEnabled(True)
        self.anim_btn.setEnabled(True)
        self.reinit_btn.setEnabled(False)  # Prevent reinit while running

    def stop_continuous_learning(self):
        if self.worker and self.worker.isRunning():
            self.log_message("Stopping learning worker...")
            self.worker.stop_learning()
        if PYQTGRAPH_AVAILABLE and hasattr(self, 'plot_timer') and self.plot_timer.isActive(): self.plot_timer.stop()

    def on_learning_stopped_ui_update(self):  # Called by signal when worker actually finishes
        self.start_btn.setEnabled(True)
        self.stop_btn.setEnabled(False)
        self.anim_btn.setEnabled(False)
    ...
    def trigger_one_sample_animation(self):
        ...
        in_ids_anim, tgt_id_anim, snippet_anim = self.val_ds[idx]  # These are CPU tensors

        self.log_message(f"Requesting animation for sample {idx}: {snippet_anim[:50]}...")
        current_hps_for_anim = self._get_current_hps_from_ui()  # Use current UI HPs for animation
        self.model.current_hps = current_hps_for_anim  # Update model's hps copy for animation delay
        self.worker.request_animation(
            (in_ids_anim, tgt_id_anim, snippet_anim))  # Pass CPU tensors; worker moves to DEVICE
    ...
    def handle_stats_update(self, stats: Dict[str, Any]):
        with self.plot_cache_lock:
            self.plot_cache = stats.copy()  # Cache for throttled plot updates
        ...
        for i, (path_tuple, count) in enumerate(sorted_paths[:20]):
        ...
        for p_tuple, c_val in sorted_paths:
            if total_paths > 0 and c_val / total_paths > 0.05:
                self.path_stats_text.append(f"MERGE: {'->'.join(map(str, p_tuple))} ({c_val / total_paths:.2%})")
                found_candidates = True
        if not found_candidates: self.path_stats_text.append("None.")

        self.log_message(f"Step {stats.get('global_step', 0)}: VL={stats.get('current_val_loss', float('nan')):.4f}, "
                         f"PPL={stats.get('current_perplexity', float('nan')):.2f}, GN={stats.get('grad_norm', float('nan')):.2f}, "
                         f"R={stats.get('reward_mean', float('nan')):.2f}")
        ...
        if PYQTGRAPH_AVAILABLE and hasattr(self, 'path_viz') and self.path_viz:
            ...
            # Define nodes: 0=input/router summary, 1 to N=experts, N+1=output/termination
            node_coords = [[0, 0, -2]]  # Input/summary slightly below center plane
            for i in range(num_exp_viz):  # Experts in a circle on Z=0 plane
                angle = 2 * math.pi * i / num_exp_viz - math.pi / 2  # Start from top
                node_coords.append([15 * math.cos(angle), 15 * math.sin(angle), 0])
            node_coords.append([0, 0, 2])  # Termination/Output slightly above center plane
            node_pos_np = np.array(node_coords, dtype=np.float32)

            # Node colors: Yellow for input, Cyan for experts, Green for output
            ...
            # Draw top 5 paths
            output_node_idx = num_exp_viz + 1  # Index of the termination/output node in node_pos_np
            for path_tuple, count_val in sorted_paths[:5]:
                # Path: Start from input_node (idx 0), then expert indices (offset by 1), end at output_node (idx N+1)
                current_path_indices = [0]  # Start at input/summary node
                for step_item in path_tuple:
                    if isinstance(step_item, int):
                        current_path_indices.append(step_item + 1)  # Expert index to node_pos_np index
                    elif step_item in ('T', 'T_max'):
                        current_path_indices.append(output_node_idx); break  # Path terminates
                if current_path_indices[-1] != output_node_idx and path_tuple[-1] not in ('T', 'T_max'):
                    # If path didn't explicitly terminate but ended, assume it goes to output (e.g. from last expert)
                    # This part might need refinement based on how paths are logged vs. visualized.
                    # For now, if it doesn't end in T/T_max, it's an incomplete path for viz.
                    pass

                if len(current_path_indices) > 1:
                    # Create (M, 2, 3) array for GLLinePlotItem directly from node_pos_np and current_path_indices
                    line_segments_pos = []
                    for i_seg in range(len(current_path_indices) - 1):
                        start_node_idx = current_path_indices[i_seg]
                        end_node_idx = current_path_indices[i_seg + 1]
                        line_segments_pos.append([node_pos_np[start_node_idx], node_pos_np[end_node_idx]])
    ...
    def update_plots_from_cache(self):  # Throttled plot updates
        if not PYQTGRAPH_AVAILABLE or not self.plot_cache: return
        with self.plot_cache_lock:
            stats = self.plot_cache.copy(); self.plot_cache = None  # Consume cache
        if not stats: return

        x_eval_steps = list(range(len(stats.get('val_loss_hist', []))))  # X-axis for validation metrics
        ...
        for curve_attr, data_key in plot_map.items():
            if hasattr(self, curve_attr):
                ...
                # Training loss might have different X-axis if eval steps != 1
    ...
    def on_input_text_anim(self, t: str):
        self.diagram.reset_highlights()
        self.diagram.highlight_node("INPUT")
        self.sample_edit.setText(f"Input: {t[:100]}...")

    def on_embedded_anim(self, r: str):
        self.diagram.highlight_node("INPUT", False)
        self.diagram.highlight_connection("INPUT", "EMBED")
        self.diagram.highlight_node("EMBED")
    ...
    def on_router_output_anim(self, probs: List[float], action_idx: int, is_term: bool):
        ...
        if is_term: self.diagram.highlight_node("LM_HEAD")  # If terminating, LM_HEAD is next conceptual step

    def on_to_expert_anim(self, idx: int, r: str):
        self.diagram.highlight_node(f"EXPERT_{idx}")
        self.diagram.set_representation_text(r)

    def on_expert_output_anim(self, idx: int, r: str):
        self.diagram.set_representation_text(r)  # Expert node remains highlighted

    def on_to_output_layer_anim(self, r: str):
        # Determine the last active processing node to deactivate it visually
        current_path_str = self.diagram.current_path_text
        last_proc_node = "ROUTER"  # Default if path is empty or unclear
        if "Path: " in current_path_str:
            elements = current_path_str.replace("Path: ", "").split("->")
            if elements:
                # Last element before potential 'T' or 'T_max'
                ...
                elif not relevant_elements and "EMBED" in self.diagram.active_elements:  # Path was just EMBED -> ROUTER -> LM_HEAD
                    last_proc_node = "ROUTER"  # From router directly to LM_HEAD
                # If path was just e.g. EMBED -> ROUTER(Terminate)
                elif len(elements) == 1 and elements[0] == 'T':  # Check if router was active
                    if "ROUTER" in self.diagram.active_elements and self.diagram.active_elements["ROUTER"] != QColor(
                            "lightblue"):
                        last_proc_node = "ROUTER"

        self.diagram.highlight_node(last_proc_node, False)  # De-highlight last processing node
        self.diagram.highlight_connection(last_proc_node, "LM_HEAD", active=True)  # Show connection to LM_HEAD
    ...
    def closeEvent(self, event):  # Ensure worker thread is stopped on window close
        self.log_message("Close event received. Shutting down worker...")
        self.stop_continuous_learning()
        if self.worker and self.worker.isRunning():
            if not self.worker.wait(5000):  # Wait for worker to finish
                self.log_message("Worker did not stop in time. Terminating.")
                self.worker.terminate()
...
if __name__ == "__main__":
    ...
    if not PYQTGRAPH_AVAILABLE:
        # A simple QMessageBox can be shown if PyQt5 is available but pyqtgraph is not.
        try:
            ...
            msg_box.setText(
                "Critical Dependency Error: pyqtgraph (and pyopengl for 3D) is required for this application to run.")
            ...
            msg_box.exec_()
        except ImportError:  # PyQt5 itself not found (less likely if this code runs)
            print("ERROR: PyQt5 is required.")
        sys.exit(1)

    try:
        import torch  # Check PyTorch import again, though device config already did.
    except ImportError:
        print("ERROR: PyTorch is required.")
        sys.exit(1)
        # Similar QMessageBox could be used here.
