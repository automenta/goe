...
def generate_synthetic_text(base_text: str, num_samples: int = 10) -> str:
    tokens = base_text.lower().split()
    templates = [
        lambda t: " ".join(t[:len(t)//2] + random.sample(t, len(t)//2)),
        lambda t: " ".join(t[::-1]),  # Reverse
        lambda t: " ".join([t[i] if random.random() > 0.2 else random.choice(t) for i in range(len(t))]),  # Random swap
        lambda t: " ".join(t + random.sample(t, len(t)//3))  # Append random subset
    ]
    synthetic_texts = [base_text]
    for _ in range(num_samples - 1):
        template = random.choice(templates)
        synthetic_texts.append(template(tokens))
    return "\n".join(synthetic_texts)
...
class GlobalAttention(nn.Module):
    def __init__(self, embed_dim: int, nhead: int, dropout: float = 0.1):
        super().__init__()
        self.self_attention = nn.MultiheadAttention(embed_dim, nhead, dropout=dropout, batch_first=True)
...
class QLearningRouter(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int, num_experts: int):
        ...
        self.q_values = collections.defaultdict(float)  # State-action Q-values

    def forward(self, x_summary: torch.Tensor, visit_counts: torch.Tensor, max_visits: int, step: int) -> Tuple[torch.Tensor, torch.Tensor]:
        logits = self.q_network(x_summary)
        ...
        # Curiosity bonus
        state_key = tuple(x_summary.mean(dim=0).detach().cpu().numpy().tolist()[:10])
        curiosity = torch.tensor([CURIOSITY_BONUS / (1 + self.q_values.get((state_key, a), 1)) for a in range(self.num_experts + 1)],
                                device=logits.device, dtype=torch.float)
        logits = logits + curiosity
...
class GoEModel(nn.Module):
    def __init__(
        ...
        max_visits_per_expert: int, max_lm_seq_len: int
    ):
        super().__init__()
        ...
        self.global_attention = GlobalAttention(embed_dim, expert_nhead)
        self.experts = nn.ModuleList([
            Expert(embed_dim, expert_nhead, expert_dim_feedforward) for _ in range(num_experts)
        ])
        self.router = QLearningRouter(embed_dim, router_hidden_dim, num_experts)
        self.output_lm_head = nn.Linear(embed_dim, vocab_size)
        self.animation_signals_obj = None
        self.is_animating_sample = False
        self.current_hps = {}
        self.path_optimizer = nn.Linear(embed_dim, num_experts + 1)
        self.expert_configs = [{'nhead': expert_nhead, 'dim_feedforward': expert_dim_feedforward} for _ in range(num_experts)]
    ...
    def evolve_experts(self, step: int):
        if step % 100 == 0 and step > 0:
            for i in range(self.num_experts):
                if random.random() < 0.2:  # 20% chance to mutate
                    self.expert_configs[i]['nhead'] = random.choice([4, 8, 16])
                    self.expert_configs[i]['dim_feedforward'] = random.choice([512, 1024, 2048])
                    new_expert = Expert(self.embed_dim, self.expert_configs[i]['nhead'], self.expert_configs[i]['dim_feedforward'])
                    self.experts[i] = new_expert.to(self.embedding.weight.device)
            print(f"Step {step}: Evolved expert configurations: {self.expert_configs}")

    def forward(self, input_ids_seq: torch.Tensor, current_sample_text: Optional[str] = None, step: int = 0) -> Tuple[torch.Tensor, torch.Tensor, List[List[Any]], torch.Tensor]:
        ...
        current_representation_seq = self.global_attention(current_representation_seq)
        ...
        epsilon = 1e-10
        rewards = []
        for path_step in range(self.max_path_len):
            ...
            router_logits, router_probs = self.router(summary_repr, visit_counts_batch[current_indices], self.max_visits_per_expert, step)
            ...
            if self.training and random.random() < 0.1:  # Epsilon-greedy exploration
                action = torch.randint(0, self.num_experts + 1, (router_probs.size(0),), device=router_probs.device)
                one_hotDecision = F.one_hot(action, num_classes=self.num_experts + 1).float()
            else:
                one_hot_decision = F.gumbel_softmax(router_logits, tau=0.5, hard=True) if self.training else F.one_hot(
            ...
            if terminated_mask.any():
                ...
                for idx in terminated_indices.tolist():
                    ...
                    state_key = tuple(summary_repr[terminated_mask].mean(dim=0).detach().cpu().numpy().tolist()[:10])
                    for i, idx in enumerate(terminated_indices.tolist()):
                        action = one_hot_decision[terminated_mask][i].argmax().item()
                        self.router.q_values[(state_key, action)] += Q_LEARNING_GAMMA * (-1.0)  # Negative reward for termination
            ...
            for i, idx in enumerate(next_indices.tolist()):
                ...
                state_key = tuple(summary_repr[i].detach().cpu().numpy().tolist()[:10])
                self.router.q_values[(state_key, expert_idx)] += Q_LEARNING_GAMMA * 0.1  # Small positive reward
        ...
        return logits, total_router_entropy, paths_taken_indices, path_loss
...
class DiagramWidget(QWidget):
    ...
    def _setup_node_positions(self):
        ...
        self.node_positions = {
            ...
            "EMBED": QPointF(W / 2, H * 0.20),
            "GLOBAL": QPointF(W / 2, H * 0.32),
            "ROUTER": QPointF(W / 2, H * 0.44)
        }
        expert_y = H * 0.60
        for i in range(self.num_experts):
        ...
        self.base_connections = [
            ("INPUT", "EMBED"), ("EMBED", "GLOBAL"), ("GLOBAL", "ROUTER")
    ...
    def highlight_node(self, name: str, active: bool = True):
        if name not in self.node_positions:
            return
        if active:
            self.active_elements[name] = QColor("cyan")
    ...
    def highlight_connection(self, from_name: str, to_name: str, active: bool = True):
        ...
        for i, (s, d, c, _) in enumerate(self.connections_to_draw):
            if s == from_name and d == to_name:
                self.connections_to_draw[i] = (s, d, QColor("magenta") if active else QColor("lightgray"), active)
                updated = True
                break
        if not updated:
            self.connections_to_draw.append((from_name, to_name, QColor("magenta") if active else QColor("lightgray"), active))
        if active:
            for i, (s, d, c, _) in enumerate(self.connections_to_draw):
                if s == from_name and d != to_name:
    ...
    def paintEvent(self, event):
        ...
        for name, pos in self.node_positions.items():
            ...
            elif "GLOBAL" in display_name:
                display_name = "GLOBAL ATTN"
        ...
        painter.setPen(Qt.black)
        painter.drawText(QPointF(10, 20), self.current_representation_text)
        painter.drawText(QPointF(10, 40), self.router_probs_text)
        painter.drawText(QPointF(10, 60), self.current_path_text)
        painter.drawText(QPointF(10, 80), self.expert_usage_text)
...
class ContinuousLearningWorker(QThread):
    ...
    def run(self):
        ...
        scheduler = CyclicLR(optimizer, base_lr=1e-5, max_lr=self.hps['learning_rate'], step_size_up=CYCLE_LR_STEP_SIZE, mode='triangular2')
        ...
        for step in range(TOTAL_STEPS):
            ...
            if self._pause_for_animation and self._animate_sample_info:
                ...
                with torch.no_grad():
                    model(ids.unsqueeze(0), current_sample_text_for_anim=txt, step=step)
            ...
            with autocast(enabled=torch.cuda.is_available()):
                logits, router_entropy, paths, aux_loss = model(input_ids_seq, step=step)
                task_loss = criterion(logits, target_ids)
                loss = task_loss - self.hps['router_entropy_coef'] * router_entropy + 0.01 * aux_loss
            ...
            if self.scaler:
                ...
                loss.backward()
            accumulated_steps += 1
            if accumulated_steps >= GRADIENT_ACCUM_STEPS:
                if self.scaler:
            ...
            self.global_step += 1
            model.evolve_experts(self.global_step)
            if self.global_step % self.hps['train_steps_per_eval'] == 0:
                ...
                with torch.no_grad():
                    for val_input_ids, val_target_ids, _ in val_loader:
                        val_logits, val_router_entropy, val_paths, _ = model(val_input_ids, step=step)
...
class MainWindow(QMainWindow):
    ...
    def _init_ui(self):
        ...
        self.num_experts_spin.setRange(2, 10)
        ...
        self.max_path_len_spin.setRange(1, 12)
    ...
    def update_nhead_options(self):
        ...
        self.expert_nhead_spin.setRange(min(valid_nheads, default=1), max(valid_nheads, default=16))
    ...
    def init_system_data_and_model(self):
        ...
        if errors:
            ...
            return
        all_tokens = tokenize_text(generate_synthetic_text(WIKITEXT_SAMPLE))
    ...
    def on_to_router_anim(self, r: str, p: List[Any]):
        self.diagram_widget.highlight_node("GLOBAL", False)
        if p and p[-1] != 'T' and p[-1] != 'T_max' and isinstance(p[-1], int):
            ...
            self.diagram_widget.highlight_connection("GLOBAL", "ROUTER")
