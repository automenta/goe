...
MAX_VOCAB_SIZE = 2000
MIN_FREQ = 2
MAX_LM_SEQ_LEN = 20  # Increased for better context
TRAIN_SPLIT_RATIO = 0.8

# Model Hyperparameters
EMBED_DIM_DEFAULT = 128  # Increased for more capacity
NUM_EXPERTS_DEFAULT = 3
EXPERT_NHEAD_DEFAULT = 4
EXPERT_DIM_FEEDFORWARD_DEFAULT = 512  # Increased for more capacity
...
BATCH_SIZE = 8
ROUTER_ENTROPY_COEF = 0.005  # Reduced to balance loss
LOAD_BALANCE_COEF = 0.01  # New: for expert usage balancing
GRADIENT_CLIP_NORM = 0.5  # Tightened for stability
WEIGHT_DECAY = 1e-4
TRAIN_STEPS_PER_EVAL = 25  # More frequent evaluations
...
MAX_PLOT_POINTS = 100
DIAGRAM_NODE_WIDTH = 90
DIAGRAM_NODE_HEIGHT = 30
DIAGRAM_ARROW_SIZE = 7.0
...
class RoutingController(nn.Module):
    ...
    def forward(self, x_summary: torch.Tensor, visit_counts: torch.Tensor, max_visits: int) -> Tuple[torch.Tensor, torch.Tensor]:
...
class GoEModel(nn.Module):
    ...
    def compute_load_balance_loss(self, router_probs: torch.Tensor) -> torch.Tensor:
        ...
        mean_prob = expert_probs.mean(dim=0)
        load_balance_loss = torch.var(mean_prob) * self.num_experts
    ...
    def forward(self, input_ids_seq: torch.Tensor, current_sample_text: Optional[str] = None) -> Tuple[torch.Tensor, torch.Tensor, List[List[Any]], torch.Tensor]:
        ...
        total_load_balance_loss = torch.tensor(0.0, device=input_ids_seq.device)
        ...
        for step in range(self.max_path_len):
            ...
            router_logits, router_probs = self.router(summary_repr, visit_counts_batch[current_indices], self.max_visits_per_expert)
            router_probs_clamped = router_probs.clamp(min=epsilon, max=1.0 - epsilon)
            step_entropy = -torch.sum(router_probs_clamped * torch.log(router_probs_clamped + epsilon), dim=1).mean()
            total_router_entropy += step_entropy
            step_load_balance_loss = self.compute_load_balance_loss(router_probs)
            total_load_balance_loss += step_load_balance_loss
        ...
        return logits, total_router_entropy, paths_taken_indices, total_load_balance_loss
...
class ContinuousLearningWorker(QThread):
    ...
    def run(self):
        ...
        while self._is_running:
            ...
            model.train()
            optimizer.zero_grad()
            logits, router_entropy, paths, load_balance_loss = model(input_ids_seq)
            task_loss = criterion(logits, target_ids)
            loss = task_loss + self.hps['router_entropy_coef'] * router_entropy + self.hps['load_balance_coef'] * load_balance_loss
            loss.backward()
            ...
            if self.global_step % self.hps['train_steps_per_eval'] == 0:
                ...
                self.signals.log_message.emit(
                    f"Step {self.global_step}: Val Loss={avg_val_loss:.4f}, PPL={perplexity:.2f}, LR={optimizer.param_groups[0]['lr']:.6f}"
                )
            QThread.msleep(1)
...
class MainWindow(QMainWindow):
    ...
    def _init_ui(self):
        ...
        self.embed_dim_spin.setRange(16, 256)
        ...
        self.expert_ff_spin.setRange(32, 1024)
    ...
    def handle_stats_update(self, stats_payload: Dict[str, Any]):
        ...
        lr = stats_payload.get('current_lr', float('nan'))
        self.log_message(f"Step {gs}: Val Loss={cl:.4f}, PPL={cp:.2f}, LR={lr:.6f}")
